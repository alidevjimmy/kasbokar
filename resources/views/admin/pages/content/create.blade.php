...
<form style="margin: auto" class="col-md-6 mt-3" action="{{ route('admin.content.store' , ['type' => $type]) }}" enctype="multipart/form-data" method="post">
    @csrf
    <div class="form-group">
        ...
        ?>
                ...
                <select name="category_id" class="form-control @error('category_id') is-invalid @enderror">
                    <option value="" disabled {{ !old('category_id') ? 'selected' : '' }}>مرحله</option>
                    @foreach($categories as $category)
                    <option value="{{ $category->id }}" {{ old('category_id') == $category->id ? 'selected' : '' }}>مرحله {{ $category->level }} - {{ $category->name }}</option>
                    @endforeach
                </select>
                @error('category_id')
                <span class="invalid-feedback" role="alert">
                ...
                <input name="preImage" value="{{ old('preImage') }}" type="file" class="form-control-file @error('preImage') is-invalid @enderror" id="preImage">
                @error('preImage')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
            ...
            ?>
                ...
                <input type="text" value="{{ old('shortText') }}" name="shortText" class="form-control @error('shortText') is-invalid @enderror" id="shortText">
                @error('shortText')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
                ...
                <select multiple name="shouldJobs[]" class="form-control @error('shouldJibs') is-invalid @enderror">
                    ...
                    <option value="EMPLOYEE" {{ old('shouldJobs') == 'EMPLOYEE' ? 'selected' : '' }}>
                        کارمند
                    </option>
                    <option value="HOMEKEEPER" {{ old('shouldJobs') == 'HOMEKEEPER' ? 'selected' : '' }}>
                        خانه دار
                    </option>
                    <option value="STUDENT" {{ old('shouldJobs') == 'STUDENT' ? 'selected' : '' }}>
                        دانشجو / دانش آموز
                    </option>
                    <option value="JOBKEEPER" {{ old('shouldJobs') == 'JOBKEEPER' ? 'selected' : '' }}>
                        جویای کار
                    </option>
                </select>
                @error('shouldJobs')
                ...
                <br />
                <label for="video">فیلم :</label>
                <input name="video" value="{{ old('video') }}" type="file" multiple class="form-control-file @error('video') is-invalid @enderror" id="video">
                @error('video')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
            ...
            ?>
                ...
                <input name="logo" value="{{ old('logo') }}" type="file" class="form-control-file @error('logo') is-invalid @enderror" id="logo">
                @error('logo')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
